...
func TestGetMoodHandler(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			requestBody: mood.RequestEntry{
				Days: 7,
			},
			userId: "user1234567890",
			mockSetup: func(svc *mocks.MockMoodService) {
				svc.On("GetMood", mock.Anything, "user1234567890", 7).Return([]mood.Entry{entry}, mocks.NoError())
			},
			expectedStatus: http.StatusOK,
			expectedBody: map[string]interface{}{
				"success": true,
		...
		},
		{
			...
			userId: "user1234567890",
			mockSetup: func(svc *mocks.MockMoodService) {
				svc.On("GetMood", mock.Anything, "user1234567890", 0).Return(nil, appError.MDInvalidDaysRange)
			},
			expectedStatus: http.StatusBadRequest,
			expectedBody: map[string]interface{}{
				"success": false,
				"status":  float64(appError.MDInvalidDaysRange.Status),
				"response": map[string]interface{}{
					"errorCode": appError.MDInvalidDaysRange.Code,
					"message":   appError.MDInvalidDaysRange.Message,
				},
			},
		},
		{
			...
			userId: "user1234567890",
			mockSetup: func(svc *mocks.MockMoodService) {
				svc.On("GetMood", mock.Anything, "user1234567890", 7).Return(nil, appError.ServerInternalError)
			},
			expectedStatus: http.StatusInternalServerError,
			expectedBody: map[string]interface{}{
				"success": false,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			mockService := new(mocks.MockMoodService)
			tt.mockSetup(mockService)
			handler := getMoodHandler(mockService)
			...
			req := httptest.NewRequest("POST", "/api/v1/mood/entries", bytes.NewBuffer(requestBody))
			req.Header.Set("Content-Type", "application/json")
...
func TestGetMoodHandler_MissingUserId(t *testing.T) {
	mockService := new(mocks.MockMoodService)
	handler := getMoodHandler(mockService)

	requestBody, _ := json.Marshal(mood.RequestEntry{Days: 7})
	req := httptest.NewRequest("POST", "/api/v1/mood/entries", bytes.NewBuffer(requestBody))
	req.Header.Set("Content-Type", "application/json")
...
func TestGetMoodHandler_EmptyUserId(t *testing.T) {
	mockService := new(mocks.MockMoodService)
	handler := getMoodHandler(mockService)
