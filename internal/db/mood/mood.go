...
func (r *repo) GetMoodByDuration(ctx context.Context, userId string, from int64) ([]mood.Entry, error) {

	var entries []mood.Entry

	err := r.db.WithContext(ctx).
		Where("user_id = ? AND created_at >= ? AND deleted_at IS NULL", userId, from).
